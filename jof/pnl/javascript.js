...
function calcPricePart(product) {
	...
	var price = 0;
	var kg = Math.round(parseFloat($('kg').value.replace(/[^0-9,]/g, '').replace(/,/, '.')));
	if(isNaN(kg)) {
		kg = 1;
	}
	
	var insurance = Math.round(parseFloat($('insurance').value.replace(/[^0-9,]/g, '').replace(/,/, '.')));
	var l = Math.round(parseFloat($('l').value.replace(/[^0-9,]/g, '').replace(/,/, '.')));
	var w = Math.round(parseFloat($('w').value.replace(/[^0-9,]/g, '').replace(/,/, '.')));
	var h = Math.round(parseFloat($('h').value.replace(/[^0-9,]/g, '').replace(/,/, '.')));
	...
	if(product == 340) {
		if($('country').value == 'GL') {
			//Grønland
			price = 141;
			price += 75*Math.round(kg);
		} else if($('country').value == 'FO') {
			//Færøerne
			price = 156;
			price += 21*Math.round(kg);
		...
		$('country').value == 'AX') {
			...
			//Amerika
			price = 156;
			price += 61*Math.round(kg);
		} else if(isAfricaAsienOrMiddeleast($('country').value)) {
			//Afrika, Asien og Mellemøsten
			price = 156;
			price += 71*Math.round(kg);
		} else {
			//Øvrige verden
			price = 156;
			price += 81*Math.round(kg);
		}
	...
	if(product == 330) {
		...
		$('country').value == 'HU') {
			//Priszone 5
			price = 196;
			price += 48*Math.round(kg);
		} else if($('country').value == 'CA' ||
		$('country').value == 'ZA' ||
		$('country').value == 'US') {
			//Priszone 6
			price = 196;
			price += 55*Math.round(kg);
		...
		$('country').value == 'TH') {
			//Priszone 7
			price = 201;
			price += 95*Math.round(kg);
		} else if($('country').value == 'FO') {
			//Priszone 8
			price = 166;
			price += 23*Math.round(kg);
		} else if($('country').value == 'GL') {
			//Priszone 9
			price = 126;
			price += 78*Math.round(kg);
		} else {
			//Øvrige lande
			price = 156;
			price += 71*Math.round(kg);
...
function validate() {
	...
	$('kg').value = Math.round(parseFloat($('kg').value.replace(/[^0-9,]/g, '').replace(/,/, '.')));
	$('l').value = Math.round(parseFloat($('l').value.replace(/[^0-9,]/g, '').replace(/,/, '.')));
	$('w').value = Math.round(parseFloat($('w').value.replace(/[^0-9,]/g, '').replace(/,/, '.')));
	$('h').value = Math.round(parseFloat($('h').value.replace(/[^0-9,]/g, '').replace(/,/, '.')));
	var insurance = Math.round(parseFloat($('insurance').value.replace(/[^0-9,]/g, '').replace(/,/, '.')));
	if(!isNaN(insurance))
