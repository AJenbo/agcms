  ...
  */
  ...
  private function Lang($key) {
    if(count($this->language) < 1) {
      $this->SetLanguage('dk'); // set the default language
    }
  ...
  public function MsgHTML($message, $basedir='') {
    ...
    if(isset($images[2])) {
      foreach($images[2] as $i => $url) {
        ...
        if (!preg_match('/^[a-z]+:\/\//Ui',$url)) {
          ...
          if ( mb_strlen($basedir) > 1 && mb_substr($basedir,-1) != '/') { $basedir .= '/'; }
          if ( mb_strlen($directory) > 1 && mb_substr($directory, -1) != '/') { $directory .= '/'; }
          if ( mb_strlen($directory) > 1 && mb_substr($directory, 0, 1) == '/') { $directory = mb_substr($directory, 1); }
    ...
    if ( empty($this->AltBody) ) {
      $this->AltBody = 'Denne mail er sendt i HTML format, hvilket din mail klient ikke understøtter!' . "\n\n";
    }
  }
