...
function init() {
	...
	contextMenuFileTile = new Proto.Menu({
	  selector: '.filetile',
	  className: 'menu desktop',
	  menuItems: filetileContextMenu
	})
	contextMenuVideoTile = new Proto.Menu({
	  selector: '.videotile',
	  className: 'menu desktop',
	  menuItems: videotileContextMenu
	})
	contextMenuswfTile = new Proto.Menu({
	  selector: '.swftile',
	  className: 'menu desktop',
	  menuItems: swftileContextMenu
	})
	contextMenuflvTile = new Proto.Menu({
	  selector: '.flvtile',
	  className: 'menu desktop',
	  menuItems: flvtileContextMenu
	})
	contextMenuImageTile = new Proto.Menu({
	  selector: '.imagetile',
	  className: 'menu desktop',
	  menuItems: imagetileContextMenu
	})
...
file.prototype.refreshThumb = function () {
	$('tilebox'+this.id).firstChild.childNodes[1].src = 'image.php?path='+encodeURIComponent(this.path)+'&maxW=128&maxH=96&timestamp='+unix_timestamp();
}
...
function renamedir(newNameObj) {
	...
	document.getElementById('loading').style.display = '';
	x_renamefile(idToDir(newNameObj.parentNode.parentNode.parentNode.id), idToDir(newNameObj.parentNode.parentNode.parentNode.id), '', newNameObj.value, renamedir_r);
}
...
function renamedir_r(data) {
	...
	if(data['error']) {
		...
		if(confirm(data['yesno'])){
			...
			document.getElementById('dir_'+data['id']+'form').firstChild.value = document.getElementById('dir_'+data['id']+'name').innerHTML;
		}
	} else {
...
function open_file_upload() {
	popUpWin('file-upload.php','file_upload','resizable=0,status=0,toolbar=0,menubar=0,location=0,scrollbars=0', 245, 127)
}
...
function renamefile(id) {
	...
	document.getElementById('loading').style.display = '';
	x_renamefile(id, files[id].path, '', document.getElementById('navn'+id+'form').firstChild.firstChild.value, renamefile_r);
}
